...
ans- What is the Difference between DBMS and RDBMS? DBMS stands for Database Management System, and RDBMS is the acronym for the Relational Database Management system. In DBMS, the data is stored as a file, whereas in RDBMS, data is stored in the form of tables.
...
ans- A data manipulation language (DML) is a computer programming language used for adding (inserting), deleting, and modifying (updating) data in a database. A DML is often a sublanguage of a broader database language such as SQL, with the DML comprising some of the operators in the language.
...
ans- Savepoint is a command in SQL that is used with the rollback command. · It is a command in Transaction Control Language that is used to mark the transaction in a save point.
